...
- Configure the usual backend environment variables for your hosting platform so the API can start:
  ...
  - Host-only values such as `enisra.com` are accepted for public URL settings and will be normalized to `https://enisra.com`. On AWS behind a proxy/load balancer, forwarded host/protocol headers are also used as a fallback when building Telegram job links.
