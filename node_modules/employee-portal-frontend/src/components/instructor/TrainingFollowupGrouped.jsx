...
const TrainingFollowupGrouped = ({
  groupedTrainingFollowups,
  cardBg,
  borderColor,
  headerBg,
  isLargerThan1024,
                      ...
                      <Wrap spacing={2} mt={1}>
                        ...
                        <WrapItem>
                          <Badge
                            ...
                            textOverflow="ellipsis"
                          >
                            {item.startDate ? new Date(item.startDate).toLocaleDateString() : "Date N/A"}
                          </Badge>
                        </WrapItem>
                        <WrapItem>
                          <Badge
                            ...
                            textOverflow="ellipsis"
                          >
                            {item.endDate ? new Date(item.endDate).toLocaleDateString() : "End N/A"}
                          </Badge>
                        </WrapItem>
                      </Wrap>
                    </Box>
                  ...
                  <Stack direction="row" spacing={4} align="center">
                    {(() => {
                      const agentNames = Array.from(
                        new Set(group.items.map((item) => item.salesAgent || item.agentName).filter(Boolean))
                      );
                    ...
                    {(() => {
                      const instructorNames = Array.from(
                        new Set(group.items.map((item) => item.assignedInstructor).filter(Boolean))
                      );
