...
import { FaUserPlus, FaUserCheck, FaTasks, FaBox } from 'react-icons/fa';
...
const KpiCards = ({ department = 'All' }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [usersRes, resourcesRes, assetsRes] = await Promise.all([
  ...
  return (
    <MotionBox variants={containerVariants} initial="hidden" animate="show">
      <SimpleGrid as={MotionBox} variants={containerVariants} columns={{ base: 2, sm: 2, md: 4 }} spacing={{ base: 3, md: 4 }} p={{ base: 2, md: 3 }}>
